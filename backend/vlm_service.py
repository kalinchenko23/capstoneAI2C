...
async def get_safe_prompt(client: AsyncOpenAI, keywords: str) -> str:
    ...
    try:
        ...
        print(f"[VLM] Error generating safe prompt: {e}")
...
async def analyze_image(
    ...
    try:
        ...
        if retry_count < MAX_RETRIES:
            ...
            print(f"[VLM] Rate limit. Retrying in {wait_time}s... (Attempt {retry_count+1}/{MAX_RETRIES})")
        ...
        if retry_count < MAX_RETRIES:
            wait_time = exponential_backoff_delay(retry_count)
            print(f"[VLM] Error '{e}'. Retrying in {wait_time}s... (Attempt {retry_count+1}/{MAX_RETRIES})")
            await asyncio.sleep(wait_time)
            return await analyze_image(client, encoded_image, safe_prompt, retry_count + 1)
...
async def generate_summary(client: AsyncOpenAI, image_descriptions: List[Dict[str, str]]) -> str:
    ...
    try:
        ...
        print("[VLM] Rate limit while generating summary.")
        return "Rate limit error: try again later."

    except AuthenticationError:
        print("[VLM] Authentication error during summary.")
        raise Exception("Authentication failed: check your API key.")
        ...
        print(f"[VLM] Summary generation error: {e}")

...
def json_to_excel(j_file):
    ...
    for row_num, location in enumerate(mydict['places'], start=1):
        ...
        data_tuple = (
            ...
            get_string(location, 'reviews_summary'), get_string(location, 'rating'),
            get_string(location, 'reviews_span'), get_string(location, 'photos_summary'),
            get_string(location, 'url_to_all_photos'), worksheet_review_link, worksheet_images_link
        )

        for col_num, value in enumerate(data_tuple):
            if isinstance(value, list):
                value = "\n".join(map(str, value))
            format_to_use = formats['link'] if col_num in [15, 16] or (isinstance(value, str) and value.startswith("http")) else formats['body']
